...
clip = mp.VideoFileClip(r"C:\Users\Lab\Documents\python itern\BBC English Masterclass The future seen from the past - Copy.mp4").subclip(0,20)
clip.audio.write_audiofile("video_audio.wav")
try :   
    import pyaudio
    import wave
    import numpy as np
...
audio = AudioSegment.from_wav("video_audio.wav") 
audio_segment=audio
...
def detect_silence(audio_segment, min_silence_len=100, silence_thresh=-25, seek_step=1):
...
def detect_nonsilent(audio_segment, min_silence_len=100, silence_thresh=-25, seek_step=1):
...
def split_on_silence(audio_segment, min_silence_len=100, silence_thresh=-25, keep_silence=100,
                     seek_step=1):
    """
...
#STANDARED SILENCE THRESH VALUE IS 100
